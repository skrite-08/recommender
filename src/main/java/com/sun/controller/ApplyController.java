...
public class ApplyController {
	...
	@RequestMapping(value = "applyingFrameLoad.domain")
	public ModelAndView applyingFrameLoad(HttpServletRequest request) {
		...
		if(userService.isHasPrivilegeAndActive(request)) {
			...
			if(userService.isApplyed(request)) {
				modelAndView.setViewName("/WEB-INF/pages/appliedFrame.jsp");
			}else {
				modelAndView.setViewName("/WEB-INF/pages/applyingFrame.jsp");
			}
	...
	@RequestMapping(value = "applyingInfo.domain")
