...
import com.sun.method.MininumHeap;
import com.sun.method.dynamic.SimilarityDynamic;
import com.sun.method.TreeNode;
                ...
                user2songRatingMatrix.forEach(new BiConsumer<Integer, float[]>() {
                    ...
                    public void accept(Integer otherUserId, float[] userRatingArray) {
                        ...
                        if(otherUserId!=curUserId) {
                            //计算当前用户和其他用户的相似性
                            float similarity=SimilarityDynamic.calculateSimilarity(user2songRatingMatrix.get(curUserId),user2songRatingMatrix.get(otherUserId));
