...
<%--    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/iconfont/iconfont.css">--%>
  ...
  <nav class="navbar navbar-expand-md navbar-light navbar-fixed-top bg-light">
        <a class="navbar-brand" href="${pageContext.request.contextPath}/home.domain">主页</a>
...
<%--            <ul class="nav nav-pills nav-justified mr-auto">--%>
            ...
            <li class="nav-item" id="trendingMusicId">
              <a class="nav-link" href="#">TrendingMusic</a>
            </li>

            <li class="nav-item" id="myMusicId">
              <a class="nav-link" href="#">MyMusic</a>
            </li>
          ...
          <form class="form-inline mt-2 mt-md-0" id="searchFormId">
			...
			<ul class="navbar-nav">
				<li id="logout" class="nav-item " <c:if test="${sessionScope.user==null}">style="display:none;"</c:if>><a
					class="nav-link " href="${pageContext.request.contextPath}/logout.domain"
	...
	$(function(){
		...
		$("#searchFormId").submit(function(){
			$('#hot').load("searchFrameLoad.domain",$('#searchFormId').serialize() );
			$("#playerId").show();
	        return false;
		});
		//matching page;
		$('#matchingId').on('click', function (e) {
			$('#hot').load("matchingFrameLoad.domain?index="+2);
		});

      $('#trendingMusicId').on('click', function (e) {
        $('#hot').load("trendingSongFrameLoad.domain");
      });
      //mySong page;
      $('#myMusicId').on('click', function (e) {
        $('#hot').load("myMusicFrameLoad.domain");
      });

		//admiring
		$('#admiringId').on('click', function (e) {
			var index=1;
			$('#hot').load("matchingFrameLoad.domain?index="+1);
		});


		//dynamics
		$('#dynamicId').on('click', function (e) {
			$('#hot').load("dynamicsFrameLoad.domain");
		});
