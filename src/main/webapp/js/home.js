...
function likingFunc(userId){
	...
	if($("#logout")[0].style.display =='none'){
		window.location.href="index.domain";
	} 
	var likingElement=$("#userId"+userId);
	var data = {        
	        "likingUserId": userId,
	 };
    url = "likingUser.domain";
	$.ajax({
        type:"POST",
        url:url,
        data:data,
 ...
 * @returns
 */
function reviewUserLoad(userId){
	$('#hot').load("reviewUserFrameLoad.domain?userId="+userId);
}
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(res.status==200){
            	...
            	$('#collapse-review-error-hint').collapse();
            	//更新最近评论区域
            	$('#newReviewAreaId').load("newReviewUserFrameLoad.domain?userId="+userId);
